...
model.compile(loss='mse', optimizer= Adam(learning_rate=learning_rate)) #Adam이라는 이름은 Adaptive Moment Estimation의 약자입니다.
...
#################################################  Adam  ###########################################
# Loss function: 모델이 얼마나 틀렸는지 측정합니다.

# Backpropagation: 가중치를 어느 방향으로 바꿔야 하는지 계산합니다.

# Optimizer: 계산된 gradient를 이용해 실제 가중치를 얼마나, 어떻게 수정할지 결정합니다.

# 즉, Adam은 모델의 학습을 진행시키는 가중치 업데이트 알고리즘입니다.
#######################################################################################################
#4 평가, 예측
...
#loss : 0.35007545351982117
#loss : 0.29725757241249084
#result = model.predict(x)

#loss : 0.274240642786026
