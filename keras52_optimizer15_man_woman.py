...
print('폴더 목록 :', os.listdir(DATA_PATH)) #os.listdir()은 지정한 폴더 안에 존재하는 # 모든 파일과 하위 폴더의 이름을 리스트 형태로 반환
# DATA_PATH : ./_data/image/man_woman/
# 폴더 목록 : ['man', 'woman']
...
man_path = os.path.join(DATA_PATH, 'man') #os.path.join()을 사용하면 운영체제에 맞는 # 폴더 경로를 안전하게 결합할 수 있습니다.
woman_path = os.path.join(DATA_PATH, 'woman') #os.path.join()을 사용하면 운영체제에 맞는 # 폴더 경로를 안전하게 결합할 수 있습니다.

man_files = [
    f for f in os.listdir(man_path) # f = man 폴더 내부의 각각의 파일 이름
    if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.gif')) #파일이 있다면 이미지 파일만 리스트에 저장됩니다, 
]                                                                    # lower()  대소문자 구분없이 사용가능.

woman_files = [
    f for f in os.listdir(woman_path)
    if f.lower().endswith(('.jpg', '.jpeg', '.png', '.bmp', '.gif'))
]

print('\n남자 이미지 :', len(man_files)) #len() 함수는 리스트에 들어 있는 데이터의 개수를 반환합니다.# 남자 이미지 : 17678
print('여자 이미지 :', len(woman_files)) #len() 함수는 리스트에 들어 있는 데이터의 개수를 # 반환합니다.# 여자 이미지 : 9489

#exit()
# ============================================================
# 3. 전체 이미지 읽기
# ============================================================
# 이미지의 크기를 100 x 100으로 변경
# 픽셀 값을 0~255 -> 0~1 사이로 변경
...
generator = datagen.flow_from_directory(
    ...
    batch_size=len(man_files) + len(woman_files),
    class_mode='binary',
    color_mode='rgb',
    shuffle=False
)
...
print('\n전체 이미지 shape :', x_all.shape)
print('전체 label shape :', y_all.shape)
print('클래스 번호 :', generator.class_indices)
...
man_class = generator.class_indices['man']
woman_class = generator.class_indices['woman']

print('\nman class   :', man_class)
print('woman class :', woman_class)


# 남자 이미지
man_index = np.where(y_all == man_class)[0]

x_man = x_all[man_index]
y_man = y_all[man_index]


# 여자 이미지
woman_index = np.where(y_all == woman_class)[0]

x_woman = x_all[woman_index]
y_woman = y_all[woman_index]


print('\n남자 데이터 :', x_man.shape)
print('여자 데이터 :', x_woman.shape)


# ============================================================
# 5. 여자 데이터 증강
# ============================================================
# 남자 데이터와 여자 데이터의 개수를 동일하게 만들기

man_count = len(x_man)
woman_count = len(x_woman)

need_count = man_count - woman_count

print('\n========================================')
print('2. 여자 데이터 증강')
print('========================================')

print('남자 데이터 수 :', man_count)
print('여자 데이터 수 :', woman_count)
print('추가할 여자 데이터 :', max(0, need_count))
...
if need_count > 0:
    ...
    x_woman_sample = x_woman[random_index]

    # 선택한 여자 이미지를 증강
    aug_generator = aug_datagen.flow(
        x_woman_sample,
        np.ones(need_count),
        batch_size=need_count,
        shuffle=False
    )

    x_woman_aug = next(aug_generator)[0]

    # 원래 여자 이미지 + 증강 여자 이미지
    x_woman = np.concatenate(
        [x_woman, x_woman_aug],
        axis=0
    )
...
print('\n증강 후 남자 데이터 :', len(x_man))
print('증강 후 여자 데이터 :', len(x_woman))
...
y = np.concatenate(
    [np.zeros(len(x_man)), np.ones(len(x_woman))],
    axis=0
)
...
# ============================================================
# 현재 Train 데이터의 20%를 Validation으로 사용
#
# 결과적으로
#
# 전체 데이터
...
print('\n========================================')
print('4. 최종 Train / Validation / Test')
print('========================================')
...
print('Train      :', len(x_train)) #Train      : 22627
print('Validation :', len(x_val))   #Validation : 5657
print('Test       :', len(x_test))  #Test       : 7072

print('Label : 0 = man, 1 = woman')  #Label : 0 = man, 1 = woman


# 여자 이미지 : 9489
# Found 27167 images belonging to 2 classes.

# 전체 이미지 shape : (27167, 100, 100, 3)
# 전체 label shape : (27167,)
# 클래스 번호 : {'man': 0, 'woman': 1}

# man class   : 0
# woman class : 1

# 남자 데이터 : (17678, 100, 100, 3)
# 여자 데이터 : (9489, 100, 100, 3)
...
# 3. 최종 데이터
# ========================================
# X shape : (35356, 100, 100, 3)
# y shape : (35356,)
# 클래스 분포 : (array([0., 1.]), array([17678, 17678], dtype=int64))

# ========================================
# 4. 최종 Train / Validation / Test
# ========================================
# Train      : (22627, 100, 100, 3) (22627,)
# Validation : (5657, 100, 100, 3) (5657,)
# Test       : (7072, 100, 100, 3) (7072,)

# Train 클래스 분포 :
# (array([0., 1.]), array([11313, 11314], dtype=int64))

# Validation 클래스 분포 :
# (array([0., 1.]), array([2829, 2828], dtype=int64))

# Test 클래스 분포 :
# (array([0., 1.]), array([3536, 3536], dtype=int64))
...
# PS C:\study> ^C
# PS C:\study> 
# PS C:\study>  c:; cd 'c:\study'; & 'C:\Users\Admin\miniconda3\envs\tf29x-gpu\python.exe' 'c:\Users\Admin\.vscode\extensions\ms-python.debugpy-2026.6.0-win32-x64\bundled\libs\debugpy\launcher' '2627' '--' 'c:\study\keras\keras51_augment5_men_women_woman_only.py' 
# ========================================
# 1. 데이터 폴더 확인
# ========================================
# DATA_PATH : ./_data/image/man_woman/
...
# Test       : 7072
# Label : 0 = man, 1 = woman
# 2026-09-22 17:58:52.702822: I tensorflow/core/platform/cpu_feature_guard.cc:193] This TensorFlow binary is optimized with oneAPI Deep Neural Network Library (oneDNN) to use the following CPU instructions in performance-critical operations:  AVX AVX2
# To enable them in other operations, rebuild TensorFlow with the appropriate compiler flags.
# 2026-09-22 17:58:54.013397: I tensorflow/core/common_runtime/gpu/gpu_device.cc:1532] Created device /job:localhost/replica:0/task:0/device:GPU:0 with 5456 MB memory:  -> device: 0, name: NVIDIA GeForce RTX 4060 Laptop GPU, pci bus id: 0000:01:00.0, compute capability: 8.9
# Model: "model"
...
#  bn2 (BatchNormalization)    (None, 100, 100, 32)      128       
                                                                 
#  pool1 (MaxPooling2D)        (None, 50, 50, 32)        0         
                                                                 
#  dropout1 (Dropout)          (None, 50, 50, 32)        0         
                                                                 
#  conv3 (Conv2D)              (None, 50, 50, 64)        18496     
                                                                 
#  bn3 (BatchNormalization)    (None, 50, 50, 64)        256       
                                                                 
#  conv4 (Conv2D)              (None, 50, 50, 64)        36928     
                                                                 
#  bn4 (BatchNormalization)    (None, 50, 50, 64)        256       
                                                                 
#  pool2 (MaxPooling2D)        (None, 25, 25, 64)        0         
                                                                 
#  dropout2 (Dropout)          (None, 25, 25, 64)        0         
                                                                 
#  conv5 (Conv2D)              (None, 25, 25, 128)       73856     
                                                                 
#  bn5 (BatchNormalization)    (None, 25, 25, 128)       512       
                                                                 
#  conv6 (Conv2D)              (None, 25, 25, 128)       147584    
                                                                 
#  bn6 (BatchNormalization)    (None, 25, 25, 128)       512       
                                                                 
#  pool3 (MaxPooling2D)        (None, 12, 12, 128)       0         
                                                                 
#  dropout3 (Dropout)          (None, 12, 12, 128)       0         
                                                                 
#  conv7 (Conv2D)              (None, 12, 12, 256)       295168    
                                                                 
#  bn7 (BatchNormalization)    (None, 12, 12, 256)       1024      
...
# =================================================================
# Total params: 618,658
# Trainable params: 616,994
# Non-trainable params: 1,664
# _________________________________________________________________
# 2026-09-22 17:58:54.553300: W tensorflow/core/framework/cpu_allocator_impl.cc:82] Allocation of 2715240000 exceeds 10% of free system memory.
# Epoch 1/50
# 2026-09-22 17:58:57.799728: I tensorflow/stream_executor/cuda/cuda_dnn.cc:384] Loaded cuDNN version 8101
# 2026-09-22 17:59:01.622275: W tensorflow/stream_executor/gpu/redzone_allocator.cc:314] INTERNAL: ptxas exited with non-zero error code -1, output: 
# Relying on driver to perform ptx compilation. 
# Modify $PATH to customize ptxas location.
# This message will be only logged once.
# 2026-09-22 17:59:03.794659: I tensorflow/stream_executor/cuda/cuda_blas.cc:1786] TensorFloat-32 will be used for the matrix multiplication. This will only be logged once.
# 227/227 [==============================] - ETA: 0s - loss: 0.5943 - accuracy: 0.72402026-09-22 17:59:22.986677: W tensorflow/core/framework/cpu_allocator_impl.cc:82] Allocation of 678840000 exceeds 10% of free system memory.
# 227/227 [==============================] - 31s 93ms/step - loss: 0.5943 - accuracy: 0.7240 - val_loss: 1.3180 - val_accuracy: 0.4999
# Epoch 2/50
# 111/227 [=============>................] - ETA: 9s - loss: 0.4183 - accuracy: 0.8068 
#exit()
# # ============================================================
# # 1. 경로 및 폴더 확인
# # ============================================================
...
# ================================ model.evaluate ================================
# 2026-09-22 18:17:05.709250: W tensorflow/core/framework/cpu_allocator_impl.cc:82] Allocation of 848640000 exceeds 10% of free system memory.
# 221/221 [==============================] - 4s 15ms/step - loss: 0.2175 - accuracy: 0.9507
# loss : 0.2175
# acc  : 0.9507
# 2026-09-22 18:17:10.376413: W tensorflow/core/framework/cpu_allocator_impl.cc:82] Allocation of 848640000 exceeds 10% of free system memory.
# accuracy_score : 0.9507
